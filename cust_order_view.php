...
$sql="SELECT * from confirmed_order as c inner join address as a on c.add_id = a.add_id inner join meals as m on c.fid = m.ID inner join rest_reg as r on m.rest_reg = r.email inner join dboy as d on d.did = a.process where c.uid = '$name'";
...
//select * from address as a inner join confirmed_order as c on a.add_id= c.add_id INNER JOIN meals as m on c.fid=m.ID where c.uid ="manishkartiks@gmail.com" and a.zip =""; 
$sql="SELECT * from confirmed_order as c inner join address as a on c.add_id = a.add_id inner join meals as m on c.fid = m.ID inner join rest_reg as r on m.rest_reg = r.email where a.process='0' and c.uid = '$name'";
$result = mysqli_query($conn, $sql);
while($row=mysqli_fetch_assoc($result)){
...
<div class="container">
  ...
  <tbody>
					...
					<tr>
					<th scope="row">No Delivery Boy Is assigned yet</th>
